...
- Follow-up fix (2026-09-23): the bypass client now signs in to the Auth emulator as `test-user` (email/password, `TEST_BYPASS_PASSWORD`) instead of emitting a local fake user. The fake never carried `request.auth`, so the emulator denied client reads/writes (`false for 'list' @ L5`). The server lazily ensures the emulator user exists (`ensureBypassUser` in `lib/firebase/server.ts`, once per process); no `firestore.rules` change.
