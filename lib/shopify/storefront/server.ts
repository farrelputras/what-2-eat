...
const SHOPIFY_STORE_DOMAIN =
  process.env.NEXT_PUBLIC_SHOPIFY_STORE_DOMAIN ?? "missing.myshopify.com";

const SHOPIFY_ACCESS_TOKEN =
  process.env.NEXT_PUBLIC_SHOPIFY_STOREFRONT_ACCESS_TOKEN ?? "missing-token";
...
export const storefront = {
  async request<Doc extends AnyStorefrontQueryString>(
    ...
    // v1 runs without Shopify env: skip the network and let callers fall back.
    if (!isShopifyConfigured()) return { data: null };
