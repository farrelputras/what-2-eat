...
export async function getShopifySitemapPagesCount(type: ShopifySitemapType): Promise<number> {
  ...
  // v1 runs without Shopify env: sitemaps render empty.
...
export async function getShopifySitemapPage(
  ...
  // v1 runs without Shopify env: generateStaticParams falls back to placeholders.
  if (!isShopifyConfigured()) return { hasNextPage: false, items: [] };
